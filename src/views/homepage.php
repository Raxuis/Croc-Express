<h3>Page d'accueil</h3>
<div class="container">
    <div class="card">
        <div class="card-header">
            <section class="slider-wrapper">
                <button class="slide-arrow slide-arrow-prev">
                    <i class="fa-solid fa-arrow-left first-carousel-arrows"></i>
                </button>

                <button class="slide-arrow slide-arrow-next">
                    <i class="fa-solid fa-arrow-right first-carousel-arrows"></i>

                </button>

                <ul class="slides-container" id="slides-container1">
                    <li class="slide slideFirst"><img src="./assets/burgers/burger_1.jpg" alt="burger_1"></li>
                    <li class="slide slideFirst"><img src="./assets/burgers/burger_1.jpg" alt="burger_1"></li>
                    <li class="slide slideFirst"><img src="./assets/burgers/burger_1.jpg" alt="burger_1"></li>
                    <li class="slide slideFirst"><img src="./assets/burgers/burger_1.jpg" alt="burger_1"></li>
                </ul>
            </section>
        </div>
        <div class="card-body">
            <h3>Burger</h3>
            <p>Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor incididunt ut labore et
                dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip
                ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu
                fugiat nulla pariatur.</p>
            <button class="other-products">Tous nos produits<i class="fa-solid fa-arrow-right"></i></button>
        </div>
    </div>
    <div class="card">
        <div class="card-header">
            <section class="slider-wrapper">
                <button class="slide-arrow slide-arrow-prev">
                    <i class="fa-solid fa-arrow-left second-carousel-arrows"></i>
                </button>

                <button class="slide-arrow slide-arrow-next">
                    <i class="fa-solid fa-arrow-right second-carousel-arrows"></i>

                </button>

                <ul class="slides-container" id="slides-container2">
                    <li class="slide slideSecond"><img src="./assets/pizzas/pizza_1.jpg" alt="pizza_1"></li>
                    <li class="slide slideSecond"><img src="./assets/pizzas/pizza_1.jpg" alt="pizza_1"></li>
                    <li class=" slide slideSecond"><img src="./assets/pizzas/pizza_1.jpg" alt="pizza_1"></li>
                    <li class="slide slideSecond"><img src="./assets/pizzas/pizza_1.jpg" alt="pizza_1"></li>
                </ul>
            </section>

        </div>
        <div class="card-body">
            <h3>Pizza</h3>
            <p>Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor incididunt ut labore et
                dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip
                ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu
                fugiat nulla pariatur.</p>
            <button class="other-products">Tous nos produits<i class="fa-solid fa-arrow-right"></i></button>
        </div>
    </div>
</div>
<script src="../src/scripts/script.js"></script>