...
function updateCart(productId, price, action, type, in_cart = true) {
  ...
  fetch(
    "../src/controllers/cart_manager_controller.php?id=" +
      productId +
      "&price=" +
      price +
      "&action=" +
              ...
              if (in_cart) {
                updateItemValue(
                  document.getElementById(
                    "item-quantity-" + type + "-" + itemId,
                  ),
                  item["quantity"],
                );
                updateTotalPriceItemValue(
                  document.getElementById(
                    "item-total-price-" + type + "-" + productId,
                  ),
                  item["quantity"],
                  price,
                );
                ...
                if (item["quantity"] === 0) {
                  document
                    .getElementById("item-" + type + "-" + productId)
                    .remove();
                  updateTotalPriceValue(
                    document.getElementById("total-price"),
                    item["totalPrice"],
...
function validateOrder() {
  fetch("../src/controllers/payment_controller.php?action=validate_order").then(
    (response) => {
      response.json().then((data) => {
        console.log(data);
      });
...
function setDeliveryStatus() {
  fetch(
    "../src/controllers/cart_manager_controller.php?action=set_delivery",
  ).then((response) => {
    response.json().then((data) => {
      console.log(data);
    });
...
function initializeDelivery() {
  ...
  delivery.addEventListener("change", (event) => {
    ...
    if (!event.target.checked) {
      updateTotalPriceValue(
        document.getElementById("total-price"),
        parseInt(document.getElementById("total-price").textContent) - 5,
      );
...
function initializeEmptyCart() {
  ...
  emptyCartButton.addEventListener("click", () => {
    fetch("../src/controllers/cart_manager_controller.php?action=clear").then(
      (response) => {
        response.json().then((data) => {
