...
function verify(confirmPassword = null, confirmPasswordLabel = null) {
    ...
    password.addEventListener('input', e => {
        ...
        if (e.target.value.length < 8) {
            passwordLabel.textContent = 'Votre mot de passe doit contenir au moins 8 caractères';
    ...
    if (confirmPassword && confirmPasswordLabel) {
        confirmPassword.addEventListener('input', e => {
            submit.hidden = true;
            if (e.target.value.length < 8) {
                confirmPasswordLabel.textContent = 'Votre mot de passe doit contenir au moins 8 caractères';
